...
To run:
	To run our Inverse Kinematics simulation, open our project in unity. Load scene and play, IK.unity. Onload, the robot will begin reaching for a target desination via one of our algorithms. Hit up and down on the keyboard to reset the arm to its original position and change what algorithm is being used. Then press the I key to start the new algorithm. The name of the solution is displayed in the top left. We move our target via the Unity editor.
...
Overview:
	...
	Jacobian PsuedoInverse works by building a matrix of all the angles and vectors of the joints in the system, inverting it, and then multiplying the matrix by a goal vector. Each frame, each rotation value is updated slightly to bring it closer and closer to the goal. This method occasionally runs into a singularity problem that halts movement of the arm.
	Damped Least Squares acts very similar to the Jacobian Inverse and actually needs to compute a Jacobian. It effects the rotation values of the joints in a very similar way. Damped Least Squares takes much longer to reach the goal, but it never encounters singularities.
	Cyclic Coordinate Descent is different from the previous solutions by rotating one joint at a time and traversing down the joint tree. It also does not need any use of matrices. CCD is much more jaggy then the other two but operates very quickly.
